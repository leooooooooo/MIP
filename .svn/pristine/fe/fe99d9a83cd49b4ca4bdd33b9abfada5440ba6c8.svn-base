//
//  NewsflashLevelOneTableViewController.m
//  Newsflash
//
...
#define kNewsflashCategoryTableViewWidth                 self.view.frame.size.width
#define kNewsflashCategoryTableViewHeight                self.view.frame.size.height
@interface NewsflashLevelOneTableViewController ()
{
...
@implementation NewsflashLevelOneTableViewController
...
-(void)loadView{
    ...
    self.tableView.frame = CGRectMake(0, 50, SCREEN_WIDTH - 70, SCREEN_HEIGHT-125);
...
- (void)viewDidLoad
{
    ...
    _customRefreshControl.headerView.frame = CGRectMake(0.0f, -170.0f, kNewsflashCategoryTableViewWidth, 170.0f);
    CGRect rect = _customRefreshControl.headerLastUpdatedLabel.frame;
    rect = CGRectMake((kNewsflashCategoryTableViewWidth-rect.size.width)/2, rect.origin.y, rect.size.width, rect.size.height);
    ...
    _fetchedResultsController = [NewsflashEntity newsflashFfetchedResultsController];
...
//    Module *module = [[Userinfo shareUserinfo] getModuleWithIdentifier:kNewsflashModuleIdentifier];
    
    [NewsflashNetRequest executeAsynchronous:@selector(getTodaySecurity) callback:@selector(callbackRequestTodaySecurity:error:) backTarget:self args:@"",nil];
    
//    [NewsflashNetRequest executeAsynchronous:@selector(getNewsflash) callback:@selector(callbackRequestCategory:error:) backTarget:self args:@"",nil];
}
...
/*!
 ...
 @result        无
 */
-(void)callbackRequestTodaySecurity:(NSData *)data error:(NSError*)error
{
    NSData *bodyData = [NewsflashNetRequest extractDataBodyFromResponsePackage:data];
    [NewsflashBLL getTodaySecurity:bodyData];
...
{
    NSData *bodyData = [NewsflashNetRequest extractDataBodyFromResponsePackage:data];
    [NewsflashBLL getNewsflashEntity:bodyData withParentEntity:nil];
    //处理完成后隐藏下拉页面
...
/*!
 ...
 @result        无
 */
- (void)configureCell:(NewsflashLevelOneCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    NewsflashEntity *entity = [_fetchedResultsController objectAtIndexPath:indexPath];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NewsflashLevelOneCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    
    if (cell == nil) {
        
        cell = [[[NewsflashLevelOneCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier] autorelease];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NewsflashEntity *category= [_fetchedResultsController objectAtIndexPath:indexPath];
