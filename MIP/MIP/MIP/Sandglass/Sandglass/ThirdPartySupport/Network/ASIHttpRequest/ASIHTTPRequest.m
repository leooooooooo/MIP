...
//add by lch begin 12-08-27
Class object_getClass();
//add by lch end    12-08-27
...
- (void)setDelegate:(id)newDelegate
{
    ...
    //add by lch begin
    _originalClass = object_getClass(newDelegate);
    //add by lch end
	[[self cancelledLock] unlock];
}
...
- (void)requestStarted
{
    ...
    if ((_originalClass && (currentClass == _originalClass)) || !_originalClass) {
...
- (BOOL)willAskDelegateForProxyCredentials
{
    ...
    id authenticationDelegate = nil;
    if ((_originalClass && (currentClass == _originalClass)) || !_originalClass) {
        
        authenticationDelegate = [self delegate];
    }
    //add by lch end
...
- (BOOL)willAskDelegateToConfirmRedirect
{
    ...
    BOOL needToAskDelegateAboutRedirect = NO;
    
    Class currentClass = object_getClass(delegate);
    if ((_originalClass && (currentClass == _originalClass)) || !_originalClass) {
        
        needToAskDelegateAboutRedirect = (([self delegate] && [[self delegate] respondsToSelector:[self willRedirectSelector]]) || ([self queue] && [[self queue] respondsToSelector:@selector(request:willRedirectToURL:)]));
    }
    //add by lch end
